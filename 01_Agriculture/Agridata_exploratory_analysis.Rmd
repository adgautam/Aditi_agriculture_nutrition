...
```{r}
agri_nopunj_nococo <- agri_nopunj_nococo %>% 
  rename(
    state = State_Name,
...
```{r}
agri_nopunj_nococo <- agri_nopunj_nococo %>% mutate(state = toupper(state))
```

Exporting the cleaned dataset
```{r}
write.csv(agri_nopunj_nococo, file = "Agridata_Cleaned_nopunjab.csv")
```
...
agri_corr %>%
  mutate( 
    case_when(
      Crop == "Sugarcane" & Crop_Year == 2011 & State_Name == "Punjab" ~  Area*1000 )
  )
```


```{r}
agri_corr %>% if(Crop("Sugarcane") & Crop_Year(2011) & State_name("Punjab")){
  Area <- agri_corr$Area*1000
  Production_by_Area <- agri_corr$Area/1000
  }
```
